...
"""
    Author: Kwong
    Create time: 2019/10/1 21:24 
"""

"""
    from github: https://github.com/songdejia/DeepLab_v3_plus/blob/master/model/deeplab.py
"""
...
def deeplab(n_class, pretrained=True):
    if pretrained:
        model = DeepLabv3_plus(nInputChannels=3, n_classes=21, os=16, pretrained=True, _print=False)
    else:
        model = DeepLabv3_plus(nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=False)
    model.last_conv = nn.Sequential(nn.Conv2d(304, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
                                       nn.ReLU(),
                                       nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                                       nn.BatchNorm2d(256),
